...
public interface TravelPlanDao {
    // The Integer type parameter tells Room to use aPositionalDataSource object,
// with position-based loading under the hood.
    @Query("SELECT * from travel_plan WHERE deleteYn=0 and travelId=:travelId ORDER BY dateTime DESC, id DESC")
            DataSource.Factory<Integer, TravelPlan> getAllPlansOfTravel(long
            travelId);
    ...
    DataSource.Factory<Integer, TravelPlan> getPlansOnDate(long travelId,
                                                           String dateTime);
