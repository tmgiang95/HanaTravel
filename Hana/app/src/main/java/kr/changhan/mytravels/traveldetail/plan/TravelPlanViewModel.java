...
public class TravelPlanViewModel extends AndroidViewModel {
    ...
    public final MutableLiveData<TravelPlan> currentItem = new
            MutableLiveData<>();
    public final LiveData<PagedList<TravelPlan>> travelPlanList
            = Transformations.switchMap(currentItem, new Function<
            TravelPlan, LiveData<PagedList<TravelPlan>>>() {
        @Override
        public LiveData<PagedList<TravelPlan>> apply(TravelPlan item) {
            return mRepository.getAllPlansOfTravel(item.getTravelId(),
                    item.getDateTime());
        }
    });
